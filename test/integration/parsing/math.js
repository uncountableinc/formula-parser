...
describe('.parse() math', () => {
  ...
  it('operator: /', () => {
    ...
    expect(parser.parse('2 / 0')).toMatchObject({error: null, result: Infinity});
    expect(parser.parse('"foo" / 4.333')).toMatchObject({error: null, result: NaN});
    expect(parser.parse('0 / 0')).toMatchObject({error: null, result: NaN});
  });
